...
export default function DetectiveGame({ apiKey, selectedModel }: DetectiveGameProps) {
  const [topic, setTopic] = useState('Tả cây bàng già góc sân trường');
  ...
  const handleGenerate = async () => {
    ...
    try {
      const res = await fetch('/api/gemini/detective', {
        ...
        body: JSON.stringify({ topic, type: 'ta-cay-coi', errorType: 'thiếu cảm xúc, lạc đề nhẹ, câu đơn điệu', model: selectedModel })
      });
