...
export async function callGeminiApiDirectly(params: DirectGeminiParams): Promise<any> {
  ...
  if (action === 'generate') {
    ...
    prompt = `Bạn là chuyên gia giáo dục tiểu học hỗ trợ dạy Tiếng Việt viết văn lớp 4.
Hãy phân tích đề bài sau và trả về phản hồi dưới dạng JSON chính xác:
Đề bài: "${params.topic}"
Dạng bài tương ứng: ${params.type} (Có thể là: ta-cay-coi, ke-chuyen-da-doc-da-nghe, cam-xuc-nhan-vat-van-hoc, thuat-lai-su-viec, neu-y-kien-hien-tuong, neu-y-kien-cau-chuyen, cam-xuc-nguoi-than, viet-thu, viet-don, ta-con-vat, viet-huong-dan-bao-cao)
...
{
  "genre": "Tên tiếng Việt hiển thị của dạng bài (ví dụ: Văn tả cây cối)",
  ...
  "errorsToAvoid": ["Danh sách 2-3 lỗi học sinh hay mắc phải đối với đề tài này"]
    ...
    responseMimeType = 'application/json';
    prompt = `Bạn là một nhà văn thiếu nhi đoạt giải thưởng lớn và là chuyên gia giáo dục Tiếng Việt lớp 4 ưu tú.
...
BẮT BUỘC TRẢ VỀ kết quả duy nhất dưới dạng một đối tượng JSON chính xác (không chứa bất kỳ chữ hay markdown nào khác ngoài khối JSON):
{
  ...
  "analysis": [
    ...
    responseMimeType = 'application/json';
    prompt = `Bạn là huấn luyện viên viết văn Tiếng Việt lớp 4 thông thái.
...
Hãy đánh giá cẩn thận và trả về cấu trúc JSON duy nhất sau (không chứa các khối markdown hay chữ khác ngoài thuộc tính JSON):
{
  ...
  "checklist": [
    ...
    responseMimeType = 'application/json';
    prompt = `Bạn là huấn luyện viên viết văn Tiếng Việt lớp 4.
...
Hãy gửi kết quả cấu trúc JSON duy nhất sau (không có các chữ nằm ngoài JSON):
{
  ...
  "feedback": {
    ...
    prompt = `Bạn là Cú Văn 🦉 — một chú cú thông thái, hài hước, thân thiện. Bạn là huấn luyện viên viết văn lớp 4.
Quy tắc:
...
Hãy trả lời bằng JSON:
{
  ...
  "suggestedOutlinePart": null hoặc { "section": "mobi|thanbi|ketbi", "content": ["ý 1", "ý 2"] }
    ...
    responseMimeType = 'application/json';
    prompt = `Bạn là huấn luyện viên viết văn lớp 4. Học sinh viết một câu đơn giản, hãy biến hóa thành 3 phiên bản hay hơn.

Câu gốc: "${params.sentence}"
Dạng bài: ${params.type || 'ta-cay-coi'}
...
Trả về JSON:
{
  ...
  "variations": [
    ...
    responseMimeType = 'application/json';
    prompt = `Bạn là giáo viên Tiếng Việt lớp 4. Hãy viết một đoạn văn ngắn (5-8 câu) có LỖI CHỦ ĐÍCH để học sinh luyện tập phát hiện lỗi.

Đề bài: "${params.topic || 'Tả cây bàng già góc sân trường'}"
Dạng bài: ${params.type || 'ta-cay-coi'}
